...
import { Props } from './CustomTable.interface';
...
const CustomTable: FC<Props> = (props) => {
  ...
  function createData(sales: any, customer: any, date: any, values: any, ostatus: any, rstatus: any, pplan: any): Data {
    return { sales, customer, date, values, ostatus, rstatus, pplan };
  }

  const dummyData = useSelector((state: RootState) => state.orders);
  ...
  const rows =
    storeData?.ordersList?.salesOrderList?.list.map((item: any) =>
      createData(
        item.saleOrder,
        item.customer,
        item.date,
  ...
  function getComparator<Key extends keyof any>(
    order: Order,
    orderBy: Key,
  ): (a: { [key in Key]: any | string }, b: { [key in Key]: any | string }) => any {
    return order === 'desc'
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n: any) => n.customer);
